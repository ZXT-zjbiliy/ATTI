...
- 在你明确点击 `应用推荐填写` 后，把可用推荐填写到页面
- 在已有推荐不变的前提下，通过 `应用推荐填写` 重新执行一次填写
...
- generic fallback 仍然只是实验性的兜底路径，只默认允许提取和预览，不默认允许页面填写，不能当成“其他网站大概率也能用”
...
2. 点击 `开始 AI 规划` 只会生成推荐并展示预览；点击 `应用推荐填写` 才会填写答案，但不会自动提交。
